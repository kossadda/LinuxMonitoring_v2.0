...
validation() {
  ...
  if [[ $# -lt 1 ]] || [[ $# -gt 3 ]]; then
    ...
    echo -e "Parameter 6: file size in megabytes - integer, range [0-100]. ${GREEN}Example: 3Mb${YELLOW}"
    code=1
  else
    ...
    if ! [[ "$3" =~ ^[0-9]+Mb$ ]] || [[ ${6::-2} -gt 100 ]] || [[ ${6::-2} -eq 0 ]]; then
      echo -e "Invalid ${RED}${6}${YELLOW} file size. Need integer, range [1-100]. ${GREEN}Example: 3Mb${YELLOW}"
      code=1
    fi
  fi
  echo -en ${RESET}
