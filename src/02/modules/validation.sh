...
validation() {
  ...
  if [[ $# -lt 1 ]] || [[ $# -gt 3 ]]; then
    ...
    echo -e "Parameter 1: letters in folder names - string [1-7].                      ${GREEN}Example: az${YELLOW}"
    echo -e "Parameter 2: letters in file names - string, name [1-7], extension [1-3]. ${GREEN}Example: az.az${YELLOW}"
    echo -e "Parameter 3: file size in megabytes - integer [0-100].                    ${GREEN}Example: 3Mb${YELLOW}"
    code=1
  else
