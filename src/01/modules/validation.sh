...
validation() {
  ...
  if [[ $# -lt 1 ]] || [[ $# -gt 6 ]]; then
    ...
    echo -e "Parameter 3: letters in folder names - string [1-7].                      ${GREEN}Example: az${YELLOW}"
    echo -e "Parameter 4: number of files in each folder - integer.                    ${GREEN}Example: 5${YELLOW}"
    echo -e "Parameter 5: letters in file names - string, name [1-7], extension [1-3]. ${GREEN}Example: az.az${YELLOW}"
    echo -e "Parameter 6: file size in kilobytes - integer [0-100].                    ${GREEN}Example: 3kb${YELLOW}"
    code=1
  else
    if ! [[ ${1} =~ ^/.*$ ]]; then
